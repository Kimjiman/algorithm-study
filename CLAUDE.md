...
## 핵심 원칙: 저작권 있는 문제 지문을 파일에 포함하지 않는다

카카오, 네이버 등 기업이 출제한 코딩테스트 문제는 저작권이 있다. 파일 생성 시 문제 지문(문제 설명, 제한 조건, 입출력 예 등)을 주석으로 복사하지 않는다.

파일에 포함해도 되는 것:

- 문제 이름, 출처 URL, 플랫폼, 유형, 복잡도, 풀이 날짜 등 메타정보
- 본인이 직접 작성한 풀이 접근 방식 설명
- 테스트케이스 입출력 값 (숫자/배열 데이터 자체)

파일에 포함하면 안 되는 것:

- 문제 본문 텍스트 (문제 설명, 게임 설명 등 서술형 지문)
- 제한 조건 원문
- 카카오/기업이 작성한 입출력 예 설명 텍스트

저작권 위험이 높은 출처: 카카오 블라인드/인턴십, 네이버, 삼성 SW 역량테스트 등 기업 출제 문제
...
- 주석(시간복잡도, 공간복잡도)이 비어있으면 **반드시 채우도록 요구**한다
- 풀이 접근 방식 주석은 풀이 완료 후 **Claude가 직접 작성**한다
